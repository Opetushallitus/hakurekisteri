  ...
  ($location, $http, $log, $q, clipboard, Opiskelijat, Suoritukset, Opiskeluoikeudet, LokalisointiService, MessageService, VirtaSuoritukset) ->
    muokkaaHenkilo: (henkiloOid, $scope) ->
      ...
      $scope.validateOppilaitoskoodiFromScopeAndUpdateMyontajaInModel = (info, model, validateError) ->
        if model.vahvistettu and not info["delete"] and info.editable and not (model.komo and (model.komo is $scope.komo.ylioppilastutkinto or isAmmatillinenKielikoe(model.komo)))
          d = $q.defer()
