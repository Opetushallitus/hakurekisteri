...
class BaseKoosteet(system: ActorSystem, integrations: Integrations, registers: Registers, config: Config) extends Koosteet {
  ...
  val hakupalvelu = new AkkaHakupalvelu(integrations.hakemusClient, integrations.hakemusService, haut)
  val hakijat = system.actorOf(Props(new HakijaActor(hakupalvelu, integrations.organisaatiot, integrations.koodisto, integrations.valintaTulos)), "hakijat")
