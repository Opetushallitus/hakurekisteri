...
class ArvosanaJournal(database: Database) extends JDBCJournal[Arvosana, ArvosanaTable, ColumnOrdered[Long], UUID] {
  ...
  override def delta(row: ArvosanaTable#TableElementType): Delta[Arvosana, UUID] =
    row match {
      case (resourceId, _, _, _, _, _, _, _,_, _, true) => Deleted(UUID.fromString(resourceId))
      case (id,suoritus, arvosana, asteikko, aine, lisatieto, valinnainen, pisteet, myonnetty,  _, _) =>
        Updated(Arvosana(UUID.fromString(suoritus), Arvio(arvosana, asteikko, pisteet), aine, lisatieto, valinnainen, myonnetty = myonnetty.map(LocalDate.parse)).identify(UUID.fromString(id)))
    }

  override def delete(id:UUID) =  currentState(id) match
  { case (foundid,suoritus, arvosana, asteikko, aine, lisatieto, valinnainen, pisteet, myonnetty,  _, _)  =>
      (foundid,suoritus, arvosana, asteikko, aine, lisatieto, valinnainen, pisteet,myonnetty, Platform.currentTime,true)}
  ...
  override def update(o:Arvosana with Identified[UUID]) = o.arvio match {
    case Arvio410(arvosana) =>
      ...
      (o.id.toString, o.suoritus.toString, arvosana, Arvio.ASTEIKKO_4_10 , o.aine, o.lisatieto, o.valinnainen, None, o.myonnetty.map(_.toString), Platform.currentTime, false)
    case ArvioYo(arvosana, pisteet) =>
      (o.id.toString, o.suoritus.toString, arvosana, Arvio.ASTEIKKO_4_10 , o.aine, o.lisatieto, o.valinnainen, Some(pisteet), o.myonnetty.map(_.toString), Platform.currentTime, false)
  ...
  override def timestamp(resource: ArvosanaTable#TableElementType): Long = resource._10
...
class ArvosanaTable(tag: Tag) extends Table[(String, String, String, String, String, Option[String], Boolean, Option[Int], Option[String], Long, Boolean)](tag, "arvosana") {
  ...
  def * = (resourceId, suoritus, arvosana, asteikko, aine, lisatieto, valinnainen, pisteet, myonnetty, inserted, deleted)
}