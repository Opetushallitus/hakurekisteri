...
trait IKoskiService {
  ...
  def updateHenkilot(oppijaOids: Set[String], createLukio: Boolean = false): Future[Unit]
...
class KoskiServiceMock extends IKoskiService {
  ...
  override def updateHenkilot(oppijaOids: Set[String], createLukio: Boolean = false): Future[Unit] = Future.successful(())
