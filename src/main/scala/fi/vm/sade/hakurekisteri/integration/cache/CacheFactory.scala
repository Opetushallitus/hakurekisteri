...
trait CacheFactory {
  ...
  def getInstance[K, T <: Serializable](
    expirationDurationMillis: Long,
    clazz: Class[_],
    classOfT: Class[_],
    cacheKeyPrefix: String
  )(implicit m: Manifest[T]): MonadCache[Future, K, T] = {
...
object CacheFactory {
  def apply(config: OphProperties)(implicit system: ActorSystem): CacheFactory =
    config.getOrElse("suoritusrekisteri.cache.redis.enabled", "false") match {
  ...
  class InMemoryCacheFactory extends CacheFactory {
    override def getInstance[K, T](
      classOfT: Class[_],
      expirationDurationMillis: Long,
      cacheKeyPrefix: String
    )(implicit m: Manifest[T]): InMemoryFutureCache[K, T] = {
      new InMemoryFutureCache[K, T](expirationDurationMillis)
    }
  }

  class RedisCacheFactory(config: OphProperties)(implicit system: ActorSystem)
    ...
    class RedisCache[K, T](
      ...
      implicit val executor: ExecutionContext =
        ExecutorUtil.createExecutor(cacheHandlingThreadPoolSize, s"cache-access-$cacheKeyPrefix")
      ...
      private val newVersion: Long = {
        ...
        try {
          ...
          case e: NullPointerException =>
            val msg =
              s"Class ${classOfT.getName} is not serializable - getSerialVersionUID resulted in NullPointerException"
            logger.error(msg)
      ...
      private def init(): Unit = {
        if (classOfT.getName.equals("scala.Option")) {
          logger.error(
            s"In cache with prefix $cacheKeyPrefix, given classOfT parameter was an Option. Please use the" +
        ...
        if (
          !runTimeTypeClass.equals(classOfT) && !runTimeTypeClass.getName.equals("scala.Option")
        ) {
          logger.warn(
            s"In cache with prefix $cacheKeyPrefix, class of type parameter T (${runTimeTypeClass.getName})" +
        ...
        val f: Future[Unit] = getVersion.flatMap {
          case Some(oldVersion) if oldVersion == newVersion =>
            logger.info(
              s"Serial version UID has not changed in cache with prefix $cacheKeyPrefix, is still $newVersion."
            )
            Future.successful(())
          case Some(oldVersion) =>
            logger.warn(
              s"Serial version UID has changed from $oldVersion to $newVersion in cache with prefix $cacheKeyPrefix. Deleting all keys."
            )
            clearCache()
          case None =>
            logger.info(
              s"No version key in cache with prefix $cacheKeyPrefix. Deleting all keys just in case the version has changed."
            )
            clearCache()
            setVersion(newVersion)
        }
        f.failed.foreach { case t =>
          throw new RedisCacheInitializationException(
            s"Failed to initialize cache with prefix $cacheKeyPrefix",
            t
          )
        }
      ...
      private def clearCache(): Future[Unit] = {
        ...
        logger.info(
          s"Finished scanning and deleting. Total number of keys successfully deleted: $totalDeleted"
        )
        setVersion(newVersion)
      }

      private def deletingScan(
        startIndex: Int,
        scanCount: Int,
        pattern: String
      ): Future[(Int, Long)] = {
        r.scan(cursor = startIndex, count = Some(scanCount), matchGlob = Some(pattern))
          .flatMap { result =>
            ...
            val fInner: Future[Long] = if (keys.nonEmpty) {
              logger.trace(
                s"SCAN returned $keysCount matching keys for PATTERN ${pattern} between indices $startIndex and ${result.index}, deleting."
              )
              r.del(keys: _*)
            } else {
              logger.trace(
                s"SCAN returned no matching keys for PATTERN ${pattern} between indices $startIndex and ${result.index}."
              )
            ...
            fInner.map { deletedCount =>
              ...
              if (keysCount != deletedCount) {
                logger.warn(
                  s"Number of keys from SCAN was $keysCount but DEL command returned $deletedCount"
                )
              }
            ...
            if (!b) {
              Future.failed(
                new RuntimeException(
                  s"Failed to store version in Redis cache with prefix $cacheKeyPrefix"
                )
              )
            } else {
              Future.successful(())
      ...
      def contains(key: K): Future[Boolean] = {
        ...
        r.exists(prefixKey).collect { case result =>
      ...
      private def get(key: K): Future[Option[T]] = {
        ...
        val f = r.get[T](prefixKey).recoverWith { case e: InvalidClassException =>
