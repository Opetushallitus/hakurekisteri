...
class ValintarekisteriActor(restClient: VirkailijaRestClient, config: Config) extends Actor {
  ...
  override def receive: Receive = {
    ...
    case ValintarekisteriQuery(personOidsWithAliases, koulutuksenAlkamiskausi) =>
      val henkiloOids =
        personOidsWithAliases.henkiloOids // Valintarekisteri already returns data for aliases
      fetchEnsimmainenVastaanotto(henkiloOids, koulutuksenAlkamiskausi) pipeTo sender
  }
  def fetchLukuvuosimaksut(
    ...
    restClient.postObject[AnyRef, Seq[Lukuvuosimaksu]]("valinta-tulos-service.lukuvuosimaksu.bulk")(
      ok,
      request
    )
  }
  def fetchEnsimmainenVastaanotto(
    henkiloOids: Set[String],
    koulutuksenAlkamiskausi: String
  ): Future[Seq[EnsimmainenVastaanotto]] = {
    restClient.postObject[Set[String], Seq[EnsimmainenVastaanotto]](
      "valinta-tulos-service.ensikertalaisuus",
...
case class ValintarekisteriQuery(
