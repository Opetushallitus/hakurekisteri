...
case class PermissionRequest(
  personOidsForSamePerson: Seq[String],
...
class HakemusBasedPermissionCheckerActor(
  hakuAppClient: VirkailijaRestClient,
  ...
  private def getOrganisationPath(organisaatio: Organisaatio): Set[String] = {
    def go(organisations: List[Organisaatio], oids: Set[String]): Set[String] =
      organisations match {
        case Nil         => oids
  ...
  private def checkHakuApp(forPerson: String, orgs: Set[String]): Future[Boolean] = {
    ...
    log.debug(
      s"Querying permissions from haku-app with ${Serialization.write(permissionRequest)(HakurekisteriJsonSupport.format)}"
    )
    hakuAppClient
      .postObject[PermissionRequest, PermissionResponse]("haku-app.permissioncheck")(
        acceptedResponseCode,
        permissionRequest
      )
      .map { permissionResponse =>
        log.debug(
          s"Got permission response for data of $forPerson from haku-app: $permissionResponse"
        )
  ...
  private def checkAtaru(forPerson: String, orgs: Set[String]): Future[Boolean] = {
    ...
    log.debug(
      s"Querying permissions from ataru with ${Serialization.write(permissionRequest)(HakurekisteriJsonSupport.format)}"
    )
    ataruClient
      .postObject[PermissionRequest, PermissionResponse]("ataru.permissioncheck")(
        acceptedResponseCode,
        permissionRequest
      )
      .map { permissionResponse =>
  ...
  private def hasPermissionFor(forPerson: String, orgs: Set[String]): Future[Boolean] = {
    log.debug(
      s"hasPermissionFor method called with arguments forPerson == $forPerson , orgs == $orgs"
    )
    Future
