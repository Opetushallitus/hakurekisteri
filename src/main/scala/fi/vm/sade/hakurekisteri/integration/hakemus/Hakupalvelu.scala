    ...
    q match {
      case HakijaQuery(Some(hakuOid), organisaatio, None, _, _, _) =>
        for {
          ...
          .map { hakemus =>
            val pohjakoulutus: Option[String] = None
            AkkaHakupalvelu.getHakija(hakemus, haku, lisakysymykset, None, pohjakoulutus)
          }
      case HakijaQuery(hakuOid, organisaatio, hakukohdekoodi, _, _, _) =>
        for {
          ...
          hakijat <- Future.sequence(for {
            ...
            val pohjakoulutus: Option[String] = None
            AkkaHakupalvelu.getHakija(hakemus, haku, lisakysymykset, hakukohdekoodi.map(_ => hakukohdeOid), pohjakoulutus)
          })
        } yield hakijat.toSeq
    }
  }
...
object AkkaHakupalvelu {
  ...
  def getHakija(hakemus: FullHakemus, haku: Haku, lisakysymykset: Map[String, ThemeQuestion], hakukohdeOid: Option[String], pohjakoulutus: Option[String]): Hakija = {
