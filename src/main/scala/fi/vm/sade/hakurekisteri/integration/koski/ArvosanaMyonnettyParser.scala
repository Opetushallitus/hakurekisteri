...
object ArvosanaMyonnettyParser {
  ...
  private def findMyonnettyToUse(
    koskiArviointiPäivä: Option[String],
    personOid: String,
    aine: String,
    suorituksenValmistumispäivä: LocalDate
  ): Option[LocalDate] = {
    Try(koskiArviointiPäivä.map(new LocalDate(_))) match {
      case Success(dateFromKoski @ Some(foundArviointiPvm))
          if foundArviointiPvm.isAfter(suorituksenValmistumispäivä) =>
        dateFromKoski
      case Failure(e) =>
        logger.warn(
          s"Virhe käsitellessä arvioinnin päivää '$koskiArviointiPäivä' hakijan '$personOid' aineelle '$aine' ." +
            s"Tallennetaan arvosanan myöntöpäiväksi tyhjä.",
          e
        )
        None
      case _ => None
