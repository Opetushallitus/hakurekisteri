...
class HenkiloActor(henkiloClient: VirkailijaRestClient)(implicit val ec: ExecutionContext) extends Actor {
  ...
  override def receive: Receive = {
    case henkiloOid: String =>
      ...
      henkiloClient.readObject[HenkiloResponse](s"/resources/henkilo/${URLEncoder.encode(henkiloOid, "UTF-8")}", HttpResponseCode.Ok) pipeTo sender

    case HetuQuery(Hetu(hetu)) =>
      logger.debug(s"received HetuQuery: ${hetu.substring(0, 6)}XXXX")
      henkiloClient.readObject[HenkiloResponse](s"/resources/s2s/byHetu/${URLEncoder.encode(hetu, "UTF-8")}", HttpResponseCode.Ok) pipeTo sender
  }
}
