...
class HttpHenkiloActor(virkailijaClient: VirkailijaRestClient, config: Config)
    extends HenkiloActor(config) {
  ...
  private val saveQueue: mutable.Map[SaveHenkilo, ActorRef] =
    new mutable.LinkedHashMap[SaveHenkilo, ActorRef]()
  private implicit val formats: Formats = DefaultFormats.lossless

  private object SaveNext

  def parseOid(h: String): String = {
    (JsonMethods.parse(h) \ "oidHenkilo").extractOpt[String].getOrElse("")
  }
  ...
  override def receive: Receive = {
    ...
    case SaveNext if !savingHenkilo && saveQueue.nonEmpty =>
      ...
      Future {
        ...
        virkailijaClient
          .postObjectWithCodes[CreateHenkilo, String](
            "oppijanumerorekisteri-service.s2s.tiedonsiirrot",
            Seq(200, 201),
            0,
            save.henkilo,
            false
          )
          .map(saved => SavedHenkilo(parseOid(saved), save.tunniste))
          .recoverWith { case t: Throwable =>
            Future.successful(HenkiloSaveFailed(save.tunniste, t))
          }
      ).pipeTo(self)(actor)
    ...
    case SaveNext if !(!savingHenkilo && saveQueue.nonEmpty) =>
      ...
      if ((currentTs - lastUnhandledSaveNext) > 10.seconds.toMillis) {
        ...
        log.debug(
          s"Received SaveNext while save in progress (savingHenkilo = ${savingHenkilo}) or save queue is empty (saveQueue.isEmpty = ${saveQueue.nonEmpty}). [Logging this message disabled for 10 seconds ]"
        )
      }
...
case class OrganisaatioHenkilo(
...
case class HenkiloSaveFailed(tunniste: String, t: Throwable)
    extends Exception(s"henkilo save failed for tunniste $tunniste", t)
