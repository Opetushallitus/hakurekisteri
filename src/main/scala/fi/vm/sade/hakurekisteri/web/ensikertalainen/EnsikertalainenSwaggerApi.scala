...
trait EnsikertalainenSwaggerApi
  ...
  val perusteFields = Seq(
    ModelField(
      ...
      null,
      DataType.String,
      allowableValues = AllowableValuesList(
        List(
          "KkVastaanotto",
          "SuoritettuKkTutkinto",
          "SuoritettuKkTutkintoHakemukselta",
          "OpiskeluoikeusAlkanut"
        )
      )
    ),
  ...
  registerModel(
    Model(
      "MenettamisenPeruste",
      "MenettamisenPeruste",
      perusteFields.map { t => (t.name, t) }.toMap
    )
  )
  ...
  val query: OperationBuilder = apiOperation[Ensikertalainen]("haeEnsikertalaisuus")
    ...
    .description(
      "Palauttaa hakijan mahdollisen ensikertalaisuuden menettämisen syyn. Jos menettämisen syytä ei ole, hakija on ensikertalainen."
    )
  ...
  val hakuQuery: OperationBuilder =
    apiOperation[Seq[Ensikertalainen]]("haeEnsikertalaisuudetHaulle")
  ...
  val postQuery: OperationBuilder = apiOperation[Seq[Ensikertalainen]]("haeEnsikertalaisuudet")
    ...
    .description(
      "Palauttaa hakijoiden mahdolliset ensikertalaisuuden menettämisen syyt. Jos menettämisen syytä ei ole, ko. hakija on ensikertalainen."
    )
