...
trait PermissionSwaggerApi extends OldSwaggerSyntax with SwaggerSupport {
  ...
  val permissionRequestFields = Seq(
    ModelField(
      ...
      "kohdehenkilön henkilöoidit",
      DataType.GenList(DataType.String)
    ),
    ModelField(
      "organisationOids",
      "virkailijan organisaatiot ja niiden lapsiorganisaatiot",
      DataType.GenList(DataType.String)
    )
  )
  ...
  def permissionRequestModel = Model(
    "PermissionCheckRequest",
    "PermissionCheckRequest",
    permissionRequestFields.map(t => (t.name, t)).toMap
  )
  def permissionResponseModel = Model(
    "PermissionCheckResponse",
    "PermissionCheckResponse",
    permissionResponseFields.map(t => (t.name, t)).toMap
  )
  def permissionErrorResponseModel = Model(
    "PermissionErrorResponse",
    "PermissionErrorResponse",
    errorResponseFields.map(t => (t.name, t)).toMap
  )
  ...
  val checkPermission = apiOperation[PermissionCheckResponse]("checkPermission")
    ...
    .description(
      "Tarkistaa onko henkilöllä käyttöoikeus johonkin listatuista organisaatioista. " +
    ...
    .responseMessage(
      ModelResponseMessage(
        504,
        "käyttöoikeustarkistusta ei ehditty tehdä määrätyssä ajassa",
        Some("PermissionErrorResponse")
      )
    )
    .responseMessage(
      ModelResponseMessage(500, "virhe käyttöoikeustarkistuksessa", Some("PermissionErrorResponse"))
    )
