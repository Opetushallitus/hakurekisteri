...
class ArvosanaResource(arvosanaActor: ActorRef, suoritusActor: ActorRef)(implicit
  sw: Swagger,
  s: Security,
  system: ActorSystem
  ...
  override def createEnabled(resource: Arvosana, user: Option[User]): Future[Boolean] =
    (suoritusActor ? resource.suoritus).mapTo[Option[Suoritus]].flatMap {
      case Some(v: VirallinenSuoritus) if v.komo == KomoOids.ammatillisenKielikoe =>
        Future.successful(user.exists(_.isAdmin))
  ...
  override def parseResourceFromBody(user: String): Either[ValidationError, Arvosana] = {
    try {
      ...
      val a = Arvosana(
        ...
        arvio = Arvio(
          arvio("arvosana").extract[String],
          arvio("asteikko").extract[String],
          arvio.get("pisteet").map(_.extract[Int])
        ),
        aine = bodyValues("aine").extract[String],
        lisatieto = bodyValues.get("lisatieto").map(_.extract[String]),
        valinnainen =
          if (bodyValues.contains("valinnainen")) bodyValues("valinnainen").extract[Boolean]
          else false,
        myonnetty =
          if (bodyValues.contains("myonnetty")) jsonToLocalDate(bodyValues("myonnetty")) else None,
