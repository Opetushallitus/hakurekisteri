...
class AjanjaksoSerializer
    extends CustomSerializer[Ajanjakso](format =>
      (
        { case ajanjakso: JObject =>
          val JString(serializedAlku) = ajanjakso \ "alku"
          val alku = DateTime.parse(serializedAlku)
          val loppu = (ajanjakso \ "loppu").toOption.collect { case JString(loppu) =>
            DateTime.parse(loppu)
          }
        ...
        },
        {
          ...
          case Ajanjakso(alku, loppu) =>
            JObject(
              JField("alku", JString(alku.toString)) :: JField(
                "loppu",
                JString(loppu.toString)
              ) :: Nil
            )
