...
class BatchOneApiCallAsMany[T](
  poolSize: Int,
  poolName: String,
  oidsToApi: Set[String] => Future[List[T]],
  oidsAtMostPerSingleApiCall: Int = 500
) extends Logging {
  ...
  private def makeApiCall: Runnable = () => {
    val works = Range
      .inclusive(1, oidsAtMostPerSingleApiCall)
      .flatMap(_ => {
        Option(stack.poll())
      })
        ...
        try {
          tryResults match {
            case Success(results) =>
              val fut = results.flatMap(o =>
                o.find(w.tester) match {
                  case Some(result) => Future.successful(result)
                  case None =>
                    Future.failed(
                      new RuntimeException(s"Couldn't find data for oid ${w.oid} in $poolName!")
                    )
                }
              )(w.executor)
              val took = w.took
