...
object Suoritus {
  ...
  def copyWithHenkiloOid(suoritus: Suoritus with Identified[UUID], henkiloOid: String)(implicit
    d: DummyImplicit
...
sealed abstract class Suoritus(val henkiloOid: String, val vahvistettu: Boolean, val source: String)
    extends UUIDResource[Suoritus] {}

case class VapaaSisalto(henkilo: String, tyyppi: String, index: Int)

case class VapaamuotoinenSuoritus(
  henkilo: String,
  kuvaus: String,
  myontaja: String,
  vuosi: Int,
  ...
  index: Int = 0,
  lahde: String
) extends Suoritus(henkilo, false, lahde) {

  val kkTutkinto: Boolean = tyyppi == "kkTutkinto"

  override val core = VapaaSisalto(henkilo, tyyppi, index)

  override def identify(identity: UUID): VapaamuotoinenSuoritus with Identified[UUID] =
    new VapaamuotoinenSuoritus(henkiloOid, kuvaus, myontaja, vuosi, tyyppi, index, source)
      with Identified[UUID] {
...
case class VirallinenSisalto(
  ...
  def isEqualWithAliases(
    otherVirallinenSisalto: VirallinenSisalto,
    aliases: Set[String]
  ): Boolean = {
    val bothOidsAreAliases =
      (aliases contains henkilo) && (aliases contains otherVirallinenSisalto.henkilo)
...
case class VirallinenSuoritus(
  ...
  override def identify(identity: UUID): VirallinenSuoritus with Identified[UUID] =
