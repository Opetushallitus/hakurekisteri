...
object Oids {
  ...
  val ammatillisetKomoOids = Set(erikoisammattitutkintoKomoOid, ammatillinentutkintoKomoOid, ammatillinenKomoOid)
}
...
class IntegrationConfig(hostQa: String, properties: Map[String, String]) {
  ...
  val koutaInternalConfig = ServiceConfig(casUrl,
    serviceUrl = koutaInternalUrl,
    serviceUser,
    servicePassword,
    properties = properties,
    maxSimultaneousConnections = findMandatoryPropertyValue("suoritusrekisteri.kouta-internal.max-connections").toInt,
    maxConnectionQueueMs = findMandatoryPropertyValue("suoritusrekisteri.kouta-internal.max-connection-queue-ms").toInt)
