...
object ExcelUtilV1 extends HakijatExcelWriter[XMLHakijat] {
  ...
  override def getHeaders(hakijat: XMLHakijat): Set[Row] = Set(
    Row(
      0,
      headers.zipWithIndex.toSet.map((header: (String, Int)) => StringCell(header._2, header._1))
    )
  )

  override def getRows(hakijat: XMLHakijat): Set[Row] = hakijat.hakijat
    .flatMap((h) =>
