...
object ExcelUtilV2 extends HakijatExcelWriter[JSONHakijat] {
  ...
  def getLisakysymysIdsAndQuestionsInOrder(hakijat: JSONHakijat) = {
    val raw: Seq[(String, String)] = hakijat.hakijat
      .flatMap(_.lisakysymykset.map(lk => lk.kysymysid -> lk.kysymysteksti))
      .distinct
      .sortBy(_._2)
  ...
  override def getRows(hakijat: JSONHakijat): Set[Row] = hakijat.hakijat
    .flatMap((h) =>
      h.hakemus.hakutoiveet.map(ht => {
        ...
        val allAnswers =
          mainAnswers ++ lisakysymysIds.map(q => getLisakysymysAnswer(h.lisakysymykset, q.id))
