...
case class Opiskelija(
  ...
  source: String
) extends UUIDResource[Opiskelija] {

  if (loppuPaiva.isDefined)
    require(
      !loppuPaiva.get.isBefore(alkuPaiva),
      "HenkiloOid: " + henkiloOid + " : loppuPaiva must be after alkuPaiva. Alku: " + alkuPaiva + ", loppu: " + loppuPaiva
    )

  override def identify(identity: UUID): Opiskelija with Identified[UUID] =
    new IdentifiedOpiskelija(this, identity)
...
class IdentifiedOpiskelija(o: Opiskelija, val id: UUID)
    extends Opiskelija(
      o.oppilaitosOid,
      o.luokkataso,
      o.luokka,
