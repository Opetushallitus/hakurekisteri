...
object OpiskelijaQuery extends Logging {
  def apply(params: Map[String,String]): OpiskelijaQuery = {
    ...
    if (personOid.isEmpty && oppilaitosOid.isEmpty) {
      logger.error(s"Both personOid and oppilaitosOid were empty, throwing an exception to avoid a too large query. Parameters were $params")
      throw new IllegalArgumentException("Joko oppijanumero ('henkilo') tai oppilaitoksen tunniste ('oppilaitosOid') on annettava. Vaikuttaa bugilta.")
    }
    OpiskelijaQuery(personOid, kausi, vuosi, paiva, oppilaitosOid, luokka)
  }
}
