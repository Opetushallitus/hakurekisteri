...
class EnsikertalainenActorSpec
  ...
  it should "return true if no kk tutkinto and no vastaanotto found" in {
    ...
    waitFuture(
      (actor ? EnsikertalainenQuery(henkiloOids = Set(henkiloOid), hakuOid = Testihaku.oid))
  ...
  it should "return ensikertalainen false based on kk tutkinto" in {
    ...
    waitFuture(
      (actor ? EnsikertalainenQuery(
        henkiloOids = Set(henkiloOid, "dummyoid"),
        hakuOid = Testihaku.oid
      )).mapTo[Seq[Ensikertalainen]]
    )((e: Seq[Ensikertalainen]) => {
      e.head.ensikertalainen should be(false)
      e.head.menettamisenPeruste should be(Some(SuoritettuKkTutkinto(date.toDateTimeAtStartOfDay)))
  ...
  it should "return ensikertalaisuus false based on hakemus" in {
    ...
    val (actor, valintarek) = initEnsikertalainenActor(
      ...
      hakemukset = Seq(
        Hakemus().setPersonOid(henkiloOid).setSuorittanutSuomalaisenKkTutkinnon(vanhatutkinto).build
      ),
      vastaanotot = Seq(
        EnsimmainenVastaanotto(
          henkiloOid,
          Some((new LocalDate().plusYears(1).toDateTimeAtCurrentTime))
        )
      )
    )
  ...
  it should "not lose ensikertalaisuus for hakemus that has suoritusoikeus_tai_aiempi_tutkinto without a year" in {
    val fullHakemus = Hakemus().setApplicationSystemId(Testihaku.oid).setPersonOid(henkiloOid).build
    val brokenAnswers = fullHakemus.answers.map { answers =>
      answers.copy(koulutustausta = answers.koulutustausta.map { koulutustausta =>
        koulutustausta.copy(
          suoritusoikeus_tai_aiempi_tutkinto = Some("true"),
  ...
  private def initEnsikertalainenActor(
    suoritukset: Seq[Suoritus] = Seq(),
    ...
    val valintarekisteri = TestActorRef(new Actor {
      var counter = 0
      override def receive: Actor.Receive = { case q: ValintarekisteriQuery =>
        counter = counter + 1
        sender ! vastaanotot
      }
    })
      ...
      system.actorOf(
        Props(
          new EnsikertalainenActor(
            ...
            opiskeluoikeusActor =
              createActorRespondingToOpiskeluoikeusHenkilotQuery(opiskeluoikeudet),
            ...
            tarjontaActor = new TarjontaActorRef(system.actorOf(Props(new Actor {
              override def receive: Actor.Receive = { case q: GetKomoQuery =>
                sender ! KomoResponse(q.oid, None)
              }
            }))),
            config = new MockConfig,
            hakuActor = system.actorOf(Props(new Actor {
              override def receive: Receive = { case q: GetHaku =>
                sender ! Testihaku
              }
            })),
  ...
  private def createActorRespondingToSuorituksetHenkilotQuery(
    suoritukset: Seq[Suoritus]
  ): ActorRef = system.actorOf(Props(new Actor {
    override def receive: Receive = { case q: SuoritusHenkilotQuery =>
      sender ! suoritukset
    }
  }))

  private def createActorRespondingToOpiskeluoikeusHenkilotQuery(
    opiskeluoikeudet: Seq[Opiskeluoikeus]
  ): ActorRef = system.actorOf(Props(new Actor {
    override def receive: Receive = { case q: OpiskeluoikeusHenkilotQuery =>
