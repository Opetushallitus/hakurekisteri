...
class ResourceAuthorizerSpec extends ScalatraFunSuite {
  ...
  private val opiskeluoikeus: Opiskeluoikeus = {
    ...
    Opiskeluoikeus(ajanjakso, "henkilooid", "komo", koulunOrgOid, "source")
  }
  val resources = Seq(opiskeluoikeus)

  test("kouluvirkailija saa lukea ja kirjoittaa organisaatioluvalla henkilöstä riippumatta") {
    authorizedResourcesForVirkailija(
      filterWithoutPerson,
      authSubjectWithOrgButNoPerson
    ) shouldBe Seq(opiskeluoikeus)
    authorizedResourcesForVirkailija(filterWithoutPerson, authSubjectWithOrgAndPerson) shouldBe Seq(
      opiskeluoikeus
    )
    authorizedResourcesForVirkailija(
      filterWithoutPerson,
      authSubjectWithOrgAndWrongPerson
    ) shouldBe Seq(opiskeluoikeus)
    ...
    authorizedResourcesForVirkailija(filterWithPerson, authSubjectWithOrgButNoPerson) shouldBe Seq(
      opiskeluoikeus
    )
    authorizedResourcesForVirkailija(filterWithPerson, authSubjectWithOrgAndPerson) shouldBe Seq(
      opiskeluoikeus
    )
    authorizedResourcesForVirkailija(
      filterWithPerson,
  ...
  test("rekisterinpitäjä saa lukea ja kirjoittaa henkilöstä riippumatta") {
    authorizedResourcesForRekisterinpitaja(
      filterWithoutPerson,
      authSubjectWithOrgButNoPerson
    ) shouldBe Seq(opiskeluoikeus)
    authorizedResourcesForRekisterinpitaja(
  ...
  test("kouluvirkailija ei saa lukea ilman oikeaa organisaatiota tai lupaa henkilöön") {
    authorizedResourcesForVirkailija(
      filterWithPerson,
      authSubjectWithWrongOrgAndNoPerson
    ) shouldBe Nil
  }

  test(
    "kouluvirkailija saa lukea mutta ei kirjoittaa väärällä organisaatioluvalla jos lupa henkilöön"
  ) {
    authorizedResourcesForVirkailija(
      filterWithPerson,
      authSubjectWithWrongOrgButCorrectPerson
    ) shouldBe Seq(opiskeluoikeus)
    authorizedResourcesForVirkailija(
      filterWithPerson,
      authSubjectWithWrongOrgButCorrectPerson,
  ...
  test("kouluvirkailija ei saa lukea luvalla väärään henkilöön") {
    authorizedResourcesForVirkailija(
      filterWithPerson,
      authSubjectWithWrongOrgAndWrongPerson
    ) shouldBe Nil
    authorizedResourcesForVirkailija(
      filterWithAnotherPerson,
      authSubjectWithWrongOrgAndNoPerson
  ...
  test("is authorized to read, write and delete in same org") {
    val ra = new ResourceAuthorizer[Opiskeluoikeus](
      filterWithoutPerson,
      mockAuthorizationSubjectFinder(authSubjectWithOrgButNoPerson)
    )
    Seq("READ", "WRITE", "DELETE").foreach { action =>
      val isAuthorizedF: Future[Boolean] =
        ra.isAuthorized(koulunVirkailijaUser, action, opiskeluoikeus)(organizationAuthorizer)
  ...
  ignore("is authorized to read from other org using auth for person") {
    val ra = new ResourceAuthorizer[Opiskeluoikeus](
      filterWithoutPerson,
      mockAuthorizationSubjectFinder(authSubjectWithWrongOrgButCorrectPerson)
    )
    val f: Future[Boolean] =
      ra.isAuthorized(koulunVirkailijaUser, "READ", opiskeluoikeus)(organizationAuthorizer)
  ...
  test("is not authorized to write or delete to other org using auth for person") {
    ...
    Seq("WRITE", "DELETE").foreach { action =>
      val isAuthorizedF: Future[Boolean] =
        ra.isAuthorized(koulunVirkailijaUser, "WRITE", opiskeluoikeus)(organizationAuthorizer)
  ...
  private def authSubjectWithWrongOrgButCorrectPerson =
    authorisationSubject(wrongOrg, correctPerson)
  private def authSubjectWithWrongOrgAndWrongPerson = authorisationSubject(wrongOrg, wrongPerson)

  private def authorisationSubject(
    personoids: Set[String],
    personOid: Option[String]
  ): Seq[AuthorizationSubject[Opiskeluoikeus]] = {
    resources.map(x =>
      AuthorizationSubject(item = x, orgs = personoids, personOid = personOid, komo = None)
    )
  }
  ...
  private def filterReturningPersonSet(
    personSet: Set[String]
  ): (User, Set[String]) => Future[Set[String]] = { (_, _) =>
    Future.successful(personSet)
  }

  private def authorizedResourcesForVirkailija(
    filter: (User, Set[String]) => Future[Set[String]],
    authorizationSubjects: Seq[AuthorizationSubject[Opiskeluoikeus]],
    action: String = "READ"
  ): Seq[Opiskeluoikeus] = {
    authorizedResources(filter, authorizationSubjects, koulunVirkailijaUser, action)
  }

  private def authorizedResourcesForRekisterinpitaja(
    filter: (User, Set[String]) => Future[Set[String]],
    authorizationSubjects: Seq[AuthorizationSubject[Opiskeluoikeus]],
    action: String = "READ"
  ): Seq[Opiskeluoikeus] = {
  ...
  private def authorizedResources(
    filter: (User, Set[String]) => Future[Set[String]],
    authorizationSubjects: Seq[AuthorizationSubject[Opiskeluoikeus]],
    virkailijaUser: User,
    action: String
  ...
  private def mockAuthorizationSubjectFinder(
    subjects: Seq[AuthorizationSubject[Opiskeluoikeus]]
  ): AuthorizationSubjectFinder[Opiskeluoikeus] = {
    new AuthorizationSubjectFinder[Opiskeluoikeus] {
      override def apply(
        v1: Seq[Opiskeluoikeus]
      ): Future[Seq[AuthorizationSubject[Opiskeluoikeus]]] = {
