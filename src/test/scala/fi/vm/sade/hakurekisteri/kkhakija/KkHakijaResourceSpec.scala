...
class KkHakijaResourceSpec extends ScalatraFunSuite with HakeneetSupport {
  ...
  test("should convert ilmoittautumiset into sequence in syksyn haku") {
    val haku = Haku(
      ...
      kausi = "kausi_s#1",
      vuosi = 2014,
      koulutuksenAlkamiskausi = "kausi_k#1",
  ...
  test("should convert ilmoittautumiset into sequence in kevään haku") {
    val haku = Haku(
      ...
      kausi = "kausi_k#1",
      vuosi = 2015,
      koulutuksenAlkamiskausi = "kausi_s#1",
      koulutuksenAlkamisvuosi = 2015,
      kkHaku = true
    )
    val sijoitteluTulos = new SijoitteluTulos {
  ...
  val haku1 = RestHaku(Some("1.2"), List(RestHakuAika(1L)), Map("fi" -> "testihaku"), "kausi_s#1", 2014, "kausi_k#1", 2015, Some("kohdejoukko_12#1"), "JULKAISTU")
