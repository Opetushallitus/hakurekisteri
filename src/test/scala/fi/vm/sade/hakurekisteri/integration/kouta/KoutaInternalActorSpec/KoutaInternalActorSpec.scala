...
import fi.vm.sade.hakurekisteri.integration.tarjonta.{HakukohteenKoulutukset, Hakukohteenkoulutus, TarjontaKoodi}
import fi.vm.sade.hakurekisteri.integration.{CapturingAsyncHttpClient, DispatchSupport, Endpoint, LocalhostProperties, ServiceConfig, VirkailijaRestClient}
...
class KoutaInternalActorSpec extends ScalatraFunSuite
  ...
  private val koodistoMock = KoodistoActorRef(system.actorOf(Props(
    new MockedKoodistoActor()
  )))
  private val koutaInternalActor = system.actorOf(Props(
  ...
  test("koutaInternalActor handles koulutukseen johtamaton koulutus") {
    Mockito.when(endPoint.request(forPattern(".*/kouta-internal/hakukohde/findbyoids")))
      .thenReturn((200, List(), getJson("koutaHakukohde")))
    Mockito.when(endPoint.request(forPattern(".*/kouta-internal/toteutus/1.2.246.562.17.00000000000000011399")))
      .thenReturn((200, List(), getJson("koutaToteutus")))
    Mockito.when(endPoint.request(forPattern(".*/kouta-internal/koulutus/1.2.246.562.13.00000000000000003172")))
