...
class VirtaQueueSpec extends WordSpec with Matchers with FutureWaiting {
  ...
  "VirtaQueue" when {
    ...
    val hakuHandler: PartialFunction[Any, Any] = {
      case q: GetHaku => Haku(Kieliversiot(Some("haku"), None, None), "1.2", Ajanjakso(new DateTime(), InFuture), "kausi_s#1", 2014, Some("kausi_k#1"), Some(2015), true, None)
    }
    ...
    "receiving query" should {
      val virtaQueue: TestActorRef[VirtaQueue] = TestActorRef[VirtaQueue](Props(new VirtaQueue(virtaActor, hakemusService, hakuActor)))
