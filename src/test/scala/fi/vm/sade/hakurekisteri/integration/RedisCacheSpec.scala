...
import fi.vm.sade.hakurekisteri.integration.cache.{
  CacheFactory,
  MonadCache,
  RedisCacheInitializationException
}
...
class RedisCacheSpec
  ...
  def redisCacheFactory(implicit system: ActorSystem): CacheFactory = CacheFactory.apply(
    new OphProperties()
  ...
  it should "add an entry to cache" in {
    withSystem(implicit system => {
      val cache = redisCacheFactory
        .getInstance[String, String](3.minutes.toMillis, this.getClass, classOf[String], "prefix1")
  ...
  it should "remove an entry from cache" in {
    withSystem(implicit system => {
      val cache = redisCacheFactory
        .getInstance[String, String](3.minutes.toMillis, this.getClass, classOf[String], "prefix2")
  ...
  it should "be usable from multiple actor systems" in {
    withSystem(implicit system => {
      val cache = redisCacheFactory
        .getInstance[String, String](3.minutes.toMillis, this.getClass, classOf[String], "prefix3")
  ...
  it should "use prefixes" in {
    withSystem(implicit system => {
      val cache = redisCacheFactory
        .getInstance[String, String](3.minutes.toMillis, this.getClass, classOf[String], "prefix4")

      Await.result(cache + (cacheKey, cacheEntry), 5.seconds)
    })
    withSystem(implicit system => {
      val cache4 = redisCacheFactory
        .getInstance[String, String](3.minutes.toMillis, this.getClass, classOf[String], "prefix4")
      val cache5 = redisCacheFactory
        .getInstance[String, String](3.minutes.toMillis, this.getClass, classOf[String], "prefix5")
  ...
  it should "only do one backend call per missing value with loader accepting get API" in {
    1.to(concurrencyTestLoopCount).foreach { counter =>
      withSystem(implicit system => {
        val cache = redisCacheFactory.getInstance[String, String](
          3.minutes.toMillis,
          this.getClass,
          classOf[String],
          s"prefixLoaderApi$counter"
        )
  ...
  it should "update version in cache when it changes" in {
    withSystem(implicit system => {
      val cacheOfStrings = redisCacheFactory
        .getInstance[String, String](3.minutes.toMillis, this.getClass, classOf[String], "prefix7")
      ...
      val cacheOfThrowables = redisCacheFactory.getInstance[String, Throwable](
        3.minutes.toMillis,
        this.getClass,
        classOf[Throwable],
        "prefix7"
      ...
      Await.result(cacheOfThrowables.getVersion, 5.seconds) should be(
        Some(throwableSerialVersionUID)
      )
    })
  }
  ...
  it should "clear cache when the version changes" in {
    withSystem(implicit system => {
      val cacheOfStrings = redisCacheFactory
        .getInstance[String, String](3.minutes.toMillis, this.getClass, classOf[String], "prefix8")
  ...
  it should "not clear cache when creating a new instance with same version" in {
    withSystem(implicit system => {
      val cache1 = redisCacheFactory
        .getInstance[String, String](3.minutes.toMillis, this.getClass, classOf[String], "prefix9")

      Await.result(cache1 + (cacheKey, cacheEntry), 5.seconds)

      cache1.shouldContain(cacheKey)

      val cache2 = redisCacheFactory
        .getInstance[String, String](3.minutes.toMillis, this.getClass, classOf[String], "prefix9")
      ...
      Await.result(cache1.get(cacheKey, stringLoader), 1.second) should be(
        Await.result(cache2.get(cacheKey, stringLoader), 1.second)
      )
    })
  }
  ...
  it should "not clear other caches when one cache changes" in {
    withSystem(implicit system => {
      val cacheOfStrings1 = redisCacheFactory
        .getInstance[String, String](3.minutes.toMillis, this.getClass, classOf[String], "prefix10")
      val cacheOfStrings2 = redisCacheFactory
        .getInstance[String, String](3.minutes.toMillis, this.getClass, classOf[String], "prefix11")
  ...
  it should "resolve the correct version for a case class option" in {
    withSystem(implicit system => {
      val expectedVersion: Long =
        java.io.ObjectStreamClass.lookup(manifest[Hakukohde].runtimeClass).getSerialVersionUID

      val cacheOfHakukohdes = redisCacheFactory.getInstance[String, Option[Hakukohde]](
        3.minutes.toMillis,
        this.getClass,
        classOf[Hakukohde],
        "prefix12"
  ...
  it should "resolve different versions for options of different case classes" in {
    withSystem(implicit system => {
      ...
      val cacheOfKoodis =
        redisCacheFactory.getInstance[String, Option[GetRinnasteinenKoodiArvoQuery]](
          3.minutes.toMillis,
          this.getClass,
          classOf[GetRinnasteinenKoodiArvoQuery],
          "prefix14"
        )
  ...
  it should "throw an exception if initializing cache with non-serializable classOfT" in {
    withSystem(implicit system => {
      intercept[RedisCacheInitializationException] {
        redisCacheFactory.getInstance[String, String](
          3.minutes.toMillis,
          this.getClass,
          classOf[ActorSystem],
          "prefix15"
  ...
  implicit class EnrichedCache[T](val cache: MonadCache[Future, String, T]) {
    ...
    def containsShouldBe(
      cache: MonadCache[Future, String, T],
      key: String,
      expected: Boolean
    ): Unit = {
