...
class KoskiArvosanaTriggerTest extends FlatSpec with Matchers with MockitoSugar {
  it should "parse a koski henkilo" in {
    ...
    val henkilo: KoskiHenkiloContainer = parse(rawJson).extract[KoskiHenkiloContainer]
    assert(henkilo != null)
    val result: Seq[(Suoritus, Seq[Arvosana], String, LocalDate, Option[String])] = KoskiArvosanaTrigger.createSuorituksetJaArvosanatFromKoski(henkilo)
    val suoritusA = result.head
    val suoritusB = result(1)

    val expectedDate = new LocalDate(2017,8,1)
    suoritusB._4 should equal (expectedDate)
    val oidsWithAliases = PersonOidsWithAliases(Set("1.2.246.562.24.71123947024"), Map.empty)
    //val f = (henkilo, oidsWithAliases)(val henkiloOid: String, val vahvistettu: Boolean, val source: String)

    val system = ActorSystem("MySpec")
    val a = system.actorOf(Props(new TestSureActor()))
    val trigger = KoskiArvosanaTrigger(a, system.actorOf(TestActors.echoActorProps), system.actorOf(TestActors.echoActorProps))

    trigger.f(henkilo, oidsWithAliases)
  }
  ...
  val rawJson = """{
  "henkilö" : {
    "oid" : "1.2.246.562.24.71123947024",
    "hetu" : "120400A184W",
    "syntymäaika" : "2000-04-12",
    "etunimet" : "TestiKiira",
    "kutsumanimi" : "TestiKiira",
    "sukunimi" : "Kiira",
    "turvakielto" : false
  },
  "opiskeluoikeudet" : [ {
    "oid" : "1.2.246.562.15.16675693036",
    "versionumero" : 15,
    "aikaleima" : "2018-04-12T12:57:17.283253",
    "oppilaitos" : {
      "oid" : "1.2.246.562.10.65693669254",
      "oppilaitosnumero" : {
        "koodiarvo" : "03096",
        "nimi" : {
          "fi" : "Ressun peruskoulu"
        },
        "lyhytNimi" : {
          "fi" : "Ressun peruskoulu"
        },
