...
class ImportBatchProcessingActorSpec extends FlatSpec with Matchers with MockitoSugar with DispatchSupport with AsyncAssertions {
  ...
  def createEndpoint = {
    ...
    when(result.request(forUrl("http://localhost/authentication-service/resources/s2s/tiedonsiirrot"))).thenReturn((200, List(), "{\"oidHenkilo\":\"1.2.246.562.24.123\"}"))
  ...
  it should "import data into henkilopalvelu and suoritusrekisteri" in {
    val batch: ImportBatch with Identified[UUID] = ImportBatch(data, Some("foo"), "perustiedot", "testitiedonsiirto").identify(UUID.randomUUID())
