...
class RekisteritiedotResourceSpec extends ScalatraFunSuite with FutureWaiting with MockitoSugar {
  ...
  val rekisterit = new Registers {
    ...
    val opiskelijat =
      system.actorOf(Props(new MockedResourceActor[Opiskelija, UUID](query = q => Seq(opiskelija))))
    val suoritukset =
      system.actorOf(Props(new MockedResourceActor[Suoritus, UUID](query = q => Seq(suoritus))))
    val ytlSuoritukset =
      system.actorOf(Props(new MockedResourceActor[Suoritus, UUID](query = q => Seq(suoritus))))
  ...
  val resource = new RekisteritiedotResource(
    rekisterit,
    mock[IHakemusService],
    notImplementedActor,
    MockOppijaNumeroRekisteri
  )
  ...
  test("oppilaitos query should match only to suoritus.myontaja") {
    val tiedot = Await.result(
      resource.fetchTiedot(
        RekisteriQuery(oppilaitosOid = Some("1.2.246.562.24.00000000001"), vuosi = None)
      ),
      15.seconds
    )
  ...
  test("correct suoritus resource id should be retained when querying with person aliases") {
    val personOids = new PersonOidsWithAliases(
