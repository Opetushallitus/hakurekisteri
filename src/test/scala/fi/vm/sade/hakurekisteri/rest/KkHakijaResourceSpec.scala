...
class KkHakijaResourceSpec extends ScalatraFunSuite with HakeneetSupport with MockitoSugar with DispatchSupport with AsyncAssertions with LocalhostProperties {
  ...
  test("should not return results if user not in hakukohde organization hierarchy") {
    ...
    val hakijat = Await.result(
      service.getKkHakijat(KkHakijaQuery(Some("1.2.246.562.24.81468276424"), None, None, None, None, Hakuehto.Kaikki, Some(testUser("test", "1.1"))), 1), 15.seconds
    )
  ...
  test("should return five hakijas") {
    ...
    val hakijat = Await.result(
      service.getKkHakijat(KkHakijaQuery(None, None, None, Some("1.2.246.562.20.649956391810"), None, Hakuehto.Kaikki, Some(testUser("test", "1.2.246.562.10.00000000001"))), 1), 150.seconds
    )
  ...
  test("should return one hyvaksytty hakija") {
    ...
    val hakijat = Await.result(
      service.getKkHakijat(KkHakijaQuery(None, None, None, Some("1.11.2"), None, Hakuehto.Hyvaksytyt, Some(testUser("test", "1.2.246.562.10.00000000001"))), 1), 15.seconds
    )
  ...
  test("should show turvakielto true from hakemus") {
    ...
    val hakijat = Await.result(service.getKkHakijat(KkHakijaQuery(Some("1.2.246.562.24.81468276424"), None, None, None, None, Hakuehto.Kaikki, Some(testUser("test", "1.2.246.562.10.00000000001"))), 1), 15.seconds)
