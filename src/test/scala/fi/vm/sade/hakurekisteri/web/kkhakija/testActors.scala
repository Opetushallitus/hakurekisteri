...
class MockedSuoritusActor(suoritus1: Suoritus) extends Actor {
  override def receive: Actor.Receive = { case q: SuoritysTyyppiQuery =>
    sender ! Seq(suoritus1)
  }
}

class MockedValintarekisteriActor(
  personOidWithLukuvuosimaksu: String,
  paymentRequiredHakukohdeWithMaksettu: String,
  noPaymentRequiredHakukohdeButMaksettu: String
) extends Actor
    with Logging {
  private val mockedMaksus: Seq[Lukuvuosimaksu] = List(
    Lukuvuosimaksu(
      personOidWithLukuvuosimaksu,
      paymentRequiredHakukohdeWithMaksettu,
      Maksuntila.maksettu,
      "muokkaaja",
      new Date()
    ),
    Lukuvuosimaksu(
      personOidWithLukuvuosimaksu,
      noPaymentRequiredHakukohdeButMaksettu,
      Maksuntila.maksettu,
      "muokkaaja2",
  ...
  override def receive: Actor.Receive = {
    case message @ LukuvuosimaksuQuery(hakukohdeOids, _)
        if hakukohdeOids.contains(paymentRequiredHakukohdeWithMaksettu) ||
          hakukohdeOids.contains(noPaymentRequiredHakukohdeButMaksettu) =>
      logger.debug(
        s"MockedValintarekisteriActor got message '$message' that matched $paymentRequiredHakukohdeWithMaksettu : returning $mockedMaksus"
      )
      sender ! mockedMaksus
    case unknown =>
      logger.debug(
        s"MockedValintarekisteriActor got unknown message '$unknown' , returning empty result"
      )
      sender ! Nil
  }
}
