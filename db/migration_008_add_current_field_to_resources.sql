...
create unique index import_batch_new_resource_id_inserted_idx on import_batch_new (resource_id, inserted);
create index i_import_batch_new_external_id on import_batch_new (external_id);
create index i_import_batch_new_state on import_batch_new (state);
create index i_import_batch_new_batch_type on import_batch_new (batch_type);
...
create unique index "opiskelija_new_resource_id_inserted_idx" on opiskelija_new (resource_id, inserted);
create index opiskelija_new_henkilo_oid_idx on opiskelija_new (henkilo_oid);
create index oppilaitos_new_oid_idx on opiskelija_new (oppilaitos_oid);
...
create unique index opiskeluoikeus_new_resource_id_inserted_idx on opiskeluoikeus_new  (resource_id, inserted);
create index "opiskeluoikeus_new_henkilo_oid_idx" on opiskeluoikeus_new (henkilo_oid);
...
create unique index suoritus_new_resource_id_inserted_idx on suoritus_new (resource_id, inserted);
create index suoritus_new_henkilo_oid_idx on suoritus_new (henkilo_oid);
create index suoritus_new_komo_idx on suoritus_new (komo);
create index suoritus_new_myontaja_idx on suoritus_new (myontaja);
