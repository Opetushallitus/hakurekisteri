...
class TiedonsiirtoTestFileGenerator extends DataGeneratorSupport {
  ...
  def valmistunutArvosanaSuoritus(name: String, date: LocalDate = new LocalDate(2015, 5,30), oppilaitosnumero: String): DataGen[Elem] = for (
    baseSuoritus <- arvosanaSuoritus(name, oppilaitosnumero)
  ...
  def arvosanaPerusopetus(oppilaitosnumero: String): DataGen[Elem] = arvosanaPerusopetus("30.5.2015", oppilaitosnumero)
  ...
  def arvosanaPerusopetus(paiva: String, oppilaitosnumero: String) =  for (
    perusopetusBase <- valmistunutArvosanaSuoritus("perusopetus", LocalDate.parse(paiva, suomalainenPaivays), oppilaitosnumero);
  ...
  def convertArvosanaPerusopetus(label: String, oppilaitosnumero: String): DataGen[Elem] = {
    for (
      perusopetusTodistus <- arvosanaPerusopetus(oppilaitosnumero)
    ) yield {
  ...
  def lisaopetus(oppilaitosnumero: String) = convertArvosanaPerusopetus("perusopetuksenlisaopetus", oppilaitosnumero)
  ...
  def arvosanaHenkilo(henkilo: (String, String, String, String), todistuksetGen: DataGen[Elem]*): DataGen[Elem] = for (
  ...
  def arvosanaSiirto(oppilaitosnumero: String, henkiloGens: DataGen[Elem]*): DataGen[Elem] =  for (
    ...
    <arvosanat xmlns:p="http://service.koodisto.sade.vm.fi/types/koodisto"
  ...
  def perustiedotSiirto(oppilaitosnumero: String, henkiloGens: DataGen[Elem]*): DataGen[Elem] = for (
    henkilot <- DataGen.combine[Elem](henkiloGens)
  ) yield
    <perustiedot xmlns:p="http://service.koodisto.sade.vm.fi/types/koodisto"
                 xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  def perustiedotHenkilo(henkilo: (String, String, String, String), lahtokoulu: String, suorituksetGen: DataGen[Elem]*): DataGen[Elem] = for (
    suoritukset <- DataGen.combine[Elem](suorituksetGen)
  ) yield
    <henkilo>
      <hetu>{henkilo._1}</hetu>
