...
CREATE OR REPLACE FUNCTION arkistoi_%%TABLE_deltat(amount integer)
  RETURNS integer AS
$BODY$
DECLARE
...
BEGIN
  FOR delta IN
    SELECT resource_id, inserted FROM %%TABLE WHERE not current
    LIMIT amount
  LOOP
...
ALTER FUNCTION arkistoi_%%TABLE_deltat(integer) OWNER TO oph;

-- invoke the function
select arkistoi_%%TABLE_deltat(:amount), :amount;
