...
class SuoritusJDBCActorSpec
  ...
  val originalSuoritus: VirallinenSuoritus = VirallinenSuoritus(
    komo = "komo",
    myontaja = "myontaja",
    tila = "KESKEN",
  ...
  it should "update suoritus with linked person oid retains original person oid in database" in {
    val original = run(
      (suoritusrekisteri ? SuoritusQuery(henkilo = Some(linkedOid2))).mapTo[Seq[VirallinenSuoritus]]
    ).head
    val newCopy = originalSuoritus.copy(henkilo = linkedOid2, tila = "VALMIS")
    run(suoritusrekisteri ? newCopy)
    val henkilo1Tilat = run(
      database.run(
        sql"select tila from suoritus where henkilo_oid = $linkedOid1 and current".as[String]
      )
    )
    val henkilo2Tilat = run(
      database.run(
        sql"select tila from suoritus where henkilo_oid = $linkedOid2 and current".as[String]
      )
    )
  ...
  it should "create new suoritus when updating field included in deduplication" in {
    val s: VirallinenSuoritus = run(
      (suoritusrekisteri ? SuoritusQuery(henkilo = Some(linkedOid2))).mapTo[Seq[VirallinenSuoritus]]
    ).head
    run(suoritusrekisteri ? s.copy(komo = "uusikomo"))
    val henkilo1Komot = run(
      database.run(
        sql"select komo from suoritus where henkilo_oid = $linkedOid1 and current".as[String]
      )
    )
    val henkilo2Komot = run(
      database.run(
        sql"select komo from suoritus where henkilo_oid = $linkedOid2 and current".as[String]
      )
    )
  ...
  it should "update suoritus with linked person oid retains original person oid in database with PersonOidsWithAliases" in {
    ...
    val newCopy = originalSuoritus.copy(henkilo = linkedOid2, tila = "VALMIS")
    run(
      suoritusrekisteri ? UpsertResource[UUID, Suoritus](
        newCopy,
        run(MockPersonAliasesProvider.enrichWithAliases(Set(linkedOid2)))
      )
    )
    val henkilo1Tilat = run(
      database.run(
  ...
  it should "update suoritus with original person oid updates suoritus with PersonOidsWithAliases" in {
    run(
      suoritusrekisteri ? UpsertResource[UUID, Suoritus](
        originalSuoritus.copy(tila = "VALMIS"),
        run(MockPersonAliasesProvider.enrichWithAliases(Set(linkedOid1)))
      )
    )
    val henkilo1Tilat = run(
      database.run(
  ...
  it should "insert new suoritus with InsertResource message" in {
    ...
    val insert = InsertResource[UUID, Suoritus](
      originalSuoritus.copy(komo = "different komo").identify,
      PersonOidsWithAliases(Set(originalSuoritus.henkiloOid))
    )
  ...
  it should "crash if InsertResource message is constructed with PersonOidsWithAliases with several oids" in {
    val brokenInsert = InsertResource[UUID, Suoritus](
      originalSuoritus.copy(komo = "different komo").identify,
      PersonOidsWithAliases(Set(originalSuoritus.henkiloOid, "1.2.3"))
    )
    val illegalArgumentException = intercept[IllegalArgumentException] {
      run(suoritusrekisteri ? brokenInsert)
    }
    illegalArgumentException.getMessage should include(
      s"Got ${brokenInsert.personOidsWithAliases.henkiloOids.size} person aliases"
    )
  }
