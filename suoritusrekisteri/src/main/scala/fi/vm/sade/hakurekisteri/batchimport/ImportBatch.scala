...
case class ImportBatch(
  ...
  batchType: String,
  source: String,
  state: BatchState = BatchState.READY,
  status: ImportStatus
) extends UUIDResource[ImportBatch] {

  override def identify(identifier: UUID): ImportBatch with Identified[UUID] =
    new IdentifiedImportBatch(this, identifier)
...
class IdentifiedImportBatch(b: ImportBatch, identifier: UUID)
    extends ImportBatch(b.data, b.externalId, b.batchType, b.source, b.state, b.status)
