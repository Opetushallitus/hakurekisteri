...
case class EnsikertalainenQuery(
...
class EnsikertalainenActor(
  ...
  val koulutuksenAlkaminenSyksy2014 =
    new DateTime(2014, 8, 1, 0, 0, 0, 0, DateTimeZone.forID("Europe/Helsinki"))
  ...
  def mergeEnsikertalaisuus(
    henkiloOid: String,
    allEnsikertalainen: Set[Ensikertalainen]
  ): Ensikertalainen = {
                  ...
                  ensikertalaisuus(
                    ...
                    haku.viimeinenHakuaikaPaattyy.getOrElse(
                      throw new IllegalArgumentException(
                        s"haku ${q.hakuOid} is missing hakuajan päätös"
                      )
                    ),
  ...
  private def tutkinnotHakemuksilta(hakuOid: String): Future[Map[String, Option[Int]]] = {
    hakemusService
      .suoritusoikeudenTaiAiemmanTutkinnonVuosi(hakuOid, None)
      .map(findCasesWithSuoritusoikeusTaiAiempiTutkinto(_).toMap)
  }

  private def tutkinnotHakemuksilta(
    henkiloOids: Set[String],
    hakuOid: String,
  ...
  private def findCasesWithSuoritusoikeusTaiAiempiTutkinto
    : Seq[HakijaHakemus] => Seq[(String, Option[Int])] = { hakemukset =>
    hakemukset.collect {
      case FullHakemus(
            _,
            Some(personOid),
      ...
      case FullHakemus(_, Some(personOid), _, _, _, _, _, _, _) =>
        (personOid, None)
      case h: AtaruHakemus =>
  ...
  private def vastaanotot(
    personOidsWithAliases: PersonOidsWithAliases
  ): Future[Map[String, DateTime]] =
    if (personOidsWithAliases.isEmpty) {
      Future.successful(Map())
    } else {
      (valintarekisterActor.actor ? ValintarekisteriQuery(personOidsWithAliases, syksy2014))
        .mapTo[Seq[EnsimmainenVastaanotto]]
        .map(_.collect { case EnsimmainenVastaanotto(oid, Some(date)) =>
          (oid, date)
        }.toMap)
    }

  private def valmistumiset(
    personOidsWithAliases: PersonOidsWithAliases,
    prefetchedSuoritukset: Seq[Suoritus]
  ): Future[Map[String, DateTime]] =
    suoritukset(personOidsWithAliases)
      .map(_ ++ prefetchedSuoritukset)
      .flatMap(kkSuoritukset)
      .map(
        _.groupBy(_.henkiloOid)
          .mapValues(aikaisinValmistuminen)
          .collect { case (henkiloOid, Some(date)) =>
            (henkiloOid, date)
          }
      )

  private def opiskeluoikeudetAlkaneet(
    personOidsWithAliases: PersonOidsWithAliases,
    prefetchedOpiskeluoikeudet: Seq[Opiskeluoikeus]
  ): Future[Map[String, DateTime]] =
  ...
  private def opiskeluoikeudet(
    personOidsWithAliases: PersonOidsWithAliases
  ): Future[Seq[Opiskeluoikeus]] = Future
    .sequence(
      personOidsWithAliases
  ...
  private def suoritukset(personOidsWithAliases: PersonOidsWithAliases): Future[Seq[Suoritus]] =
    Future
      .sequence(
        personOidsWithAliases
          .grouped(resourceQuerySize)
          .map(oidsWithAliases =>
            (suoritusActor ? SuoritusHenkilotQuery(oidsWithAliases)).mapTo[Seq[Suoritus]]
          )
      )
      .map(_.flatten.toSeq)
  ...
  private def isKkTutkinto(suoritus: Suoritus): Future[Option[Suoritus]] = suoritus match {
    case s: VirallinenSuoritus =>
      s.komo match {
        ...
        case Oid(komo) =>
          (tarjontaActor.actor ? GetKomoQuery(komo))
            .mapTo[KomoResponse]
            .flatMap(_.komo match {
  ...
  private def aikaisinValmistuminen(suoritukset: Seq[Suoritus]): Option[DateTime] = {
    val valmistumishetket = suoritukset.collect {
      case VirallinenSuoritus(_, _, "VALMIS", valmistuminen, _, _, _, _, _, _, _, _, _) =>
        valmistuminen.toDateTimeAtStartOfDay
    }
    if (valmistumishetket.isEmpty) None else Some(valmistumishetket.minBy(_.getMillis))
  }

  private def aikaisinMerkitsevaOpiskeluoikeus(
    suoritukset: Seq[Opiskeluoikeus]
  ): Option[DateTime] = {
  ...
  def ensikertalaisuus(
    ...
    (valmistuminen, opiskeluoikeusAlkanut, vastaanotto, suorittanutTutkinnonHakemukselta) match {
      ...
      case (_, Some(opiskeluoikeusAlkupvm), _, _)
          if opiskeluoikeusAlkupvm.isBefore(leikkuripaiva) =>
