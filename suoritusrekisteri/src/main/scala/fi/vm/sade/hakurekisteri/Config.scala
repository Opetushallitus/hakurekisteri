...
object KomoOids {
  ...
  val toisenAsteenVirkailijanKoulutukset = Set(
    ...
    erikoisammattitutkintoKomoOid,
    pohjakoulutus.perusopetus,
    pohjakoulutus.lukio,
    pohjakoulutus.ammatillinen,
    pohjakoulutus.ulkomainen,
    lisapistekoulutus.kymppi,
    lisapistekoulutus.talous,
    lisapistekoulutus.ammattistartti,
    lisapistekoulutus.kansanopisto,
    lisapistekoulutus.maahanmuuttajienLukioonValmistava,
...
object Oids {
  ...
  val perusopetuksenOppiaineenOppimaaraOid =
    KomoOids.pohjakoulutus.perusopetuksenOppiaineenOppimaara
  ...
  val ammatilliseenvalmistavaKomoOid =
    KomoOids.lisapistekoulutus.maahanmuuttajienAmmatilliseenValmistava
...
class DefaultConfig extends Config {
  ...
  override val databaseUrl = getPropertyOrCrash(
    "suoritusrekisteri.db.url",
    "configuration key missing: suoritusrekisteri.db.url"
  )
  override val databaseHost = getPropertyOrCrash(
    "suoritusrekisteri.db.host",
    "configuration key missing: suoritusrekisteri.db.host"
  )
  override val databasePort = getPropertyOrCrash(
    "suoritusrekisteri.db.port",
    "configuration key missing: suoritusrekisteri.db.port"
  )
  ...
  override val archiveNonCurrentAfterDays =
    properties.getOrElse("suoritusrekisteri.db.archiveNonCurrentAfterDays", "180")
  override val archiveTotalTimeoutMinutes =
    properties.getOrElse("suoritusrekisteri.db.archiveTotalTimeoutMinutes", "180")
  override val archiveBatchSize =
    properties.getOrElse("suoritusrekisteri.db.archiveBatchSize", "100000")
  override val slowQuery: Long = java.lang.Long.parseLong(
    getPropertyOrCrash(
      "suoritusrekisteri.db.slowquery.millis",
      "configuration key missing: suoritusrekisteri.db.slowquery.millis"
    )
  )
  override val reallySlowQuery: Long = java.lang.Long.parseLong(
    getPropertyOrCrash(
      "suoritusrekisteri.db.slowquery.millis",
      "configuration key missing: suoritusrekisteri.db.reallyslowquery.millis"
    )
  )
  override val maxDbLogLineLength: Int = java.lang.Integer.parseInt(
    getPropertyOrCrash(
      "suoritusrekisteri.db.max.log.line.length",
      "configuration key missing: suoritusrekisteri.db.max.log.line.length"
    )
  )
  private lazy val homeDir = sys.props.getOrElse("user.home", "")
  lazy val ophConfDir: Path = Paths.get(homeDir, "/oph-configuration/")
  override val valintaTulosTimeout: FiniteDuration = java.lang.Integer
    .parseInt(
      getPropertyOrCrash(
        "suoritusrekisteri.valintatulos.max.minutes",
        "configuration key missing: suoritusrekisteri.valintatulos.max.minutes"
      )
    )
    .minutes
  override val ytlSyncTimeout = Timeout(
    getPropertyOrCrash(
      "suoritusrekisteri.ytl.sync.timeout.seconds",
      "configuration key missing: suoritusrekisteri.ytl.sync.timeout.seconds"
    ).toLong,
    SECONDS
  )
  override val ytlSyncRetries = getPropertyOrCrash(
    "suoritusrekisteri.ytl.sync.retries",
    "configuration key missing: suoritusrekisteri.ytl.sync.retries"
  ).toInt
  override val ytlSyncParallelism: Int = getPropertyOrCrash(
    "suoritusrekisteri.ytl.sync.parallelism",
    "configuration key missing: suoritusrekisteri.ytl.sync.parallelism"
  ).toInt
}
...
class MockDevConfig extends Config {
  ...
  override val databaseUrl = properties.getOrElse(
    "suoritusrekisteri.db.url",
    "jdbc:postgresql://localhost:5432/suoritusrekisteri"
  )
  ...
  override val postgresPassword = properties.getOrElse("suoritusrekisteri.db.password", "postgres")
  override val archiveNonCurrentAfterDays =
    properties.getOrElse("suoritusrekisteri.db.archiveNonCurrentAfterDays", "14")
  override val archiveTotalTimeoutMinutes =
    properties.getOrElse("suoritusrekisteri.db.archiveTotalTimeoutMinutes", "180")
  override val archiveBatchSize =
    properties.getOrElse("suoritusrekisteri.db.archiveBatchSize", "14")
  ...
  override val ytlSyncParallelism: Int =
    properties.getOrElse("suoritusrekisteri.ytl.sync.parallelism", "10").toInt
...
abstract class Config {
  ...
  val maxPersonOidCountForHakemusBasedPermissionCheck: Int =
    java.lang.Integer.parseInt(
      getPropertyOrCrash(
        "suoritusrekisteri.hakemuspermissioncheck.max.personoids",
  ...
  def loadProperties(resources: Seq[InputStream]): Map[String, String] = {
    ...
    val rawMap = resources
      .map((reader) => {
        val prop = new java.util.Properties; prop.load(reader); Map(prop.asScala.toList: _*)
      })
  ...
  def resolve(source: Map[String, String]): Map[String, String] = {
    ...
    val unResolved = Set(
      converted
        .map((s) =>
          (for (found <- "€\\{(.*?)\\}".r findAllMatchIn s._2) yield found.group(1)).toList
        )
        .foldLeft(List[String]())(_ ++ _): _*
    )
    ...
    if ((unResolved -- unResolvable).isEmpty)
      ...
      resolve(
        converted.mapValues((s) =>
          "€\\{(.*?)\\}".r replaceAllIn (s, m => {
            converted.getOrElse(m.group(1), "€{" + m.group(1) + "}")
          })
        )
      )
  }
...
class IntegrationConfig(hostQa: String, properties: Map[String, String]) {
  ...
  val suorituspalveluUrlQa = s"https://$hostQa/suorituspalvelu"
  ...
  val supaServiceUrl = properties.getOrElse("cas.service.suorituspalvelu", suorituspalveluUrlQa)
  ...
  val virtaServiceUrl =
    properties.getOrElse("suoritusrekisteri.virta.service.url", virtaServiceUrlTest)
  val virtaJarjestelma =
    properties.getOrElse("suoritusrekisteri.virta.jarjestelma", virtaJarjestelmaTest)
  ...
  val koskiMaxOppijatPostSize = findMandatoryPropertyValue(
    "suoritusrekisteri.koski.max.oppijat.post.size"
  ).toInt
  val koskiMaxOppijatBatchSize = findMandatoryPropertyValue(
    "suoritusrekisteri.koski.max.oppijat.batch.size"
  ).toInt

  val oppijaNumeroRekisteriMaxOppijatBatchSize = findMandatoryPropertyValue(
    "suoritusrekisteri.oppijanumerorekisteri-service.max.oppijat.batch.size"
  ).toInt

  val virtaConfig =
    VirtaConfig(virtaServiceUrl, virtaJarjestelma, virtaTunnus, virtaAvain, properties)
  val parameterConfig = ServiceConfig(
    serviceUrl = parameterServiceUrl,
    properties = properties,
    maxSimultaneousConnections = findMandatoryPropertyValue(
      "suoritusrekisteri.ohjausparametrit-service.max-connections"
    ).toInt,
    maxConnectionQueueMs = findMandatoryPropertyValue(
      "suoritusrekisteri.ohjausparametrit-service.max-connection-queue-ms"
    ).toInt
  )
  val hakemusConfig = HakemusConfig(
    ServiceConfig(
      casUrl,
      hakuappServiceUrl,
      serviceUser,
      ...
      maxSimultaneousConnections =
        findMandatoryPropertyValue("suoritusrekisteri.haku-app.max-connections").toInt,
      maxConnectionQueueMs =
        findMandatoryPropertyValue("suoritusrekisteri.haku-app.max-connection-queue-ms").toInt
    ),
    maxApplications
  )
  val ataruConfig = ServiceConfig(
    casUrl,
    ataruUrl,
    serviceUser,
    servicePassword,
    properties,
    maxSimultaneousConnections =
      findMandatoryPropertyValue("suoritusrekisteri.ataru.max-connections").toInt,
    maxConnectionQueueMs =
  ...
  val koosteConfig = ServiceConfig(
    ...
    properties,
    maxSimultaneousConnections = findMandatoryPropertyValue(
      "suoritusrekisteri.valintalaskentakoostepalvelu.max-connections"
    ).toInt,
    maxConnectionQueueMs = findMandatoryPropertyValue(
      "suoritusrekisteri.valintalaskentakoostepalvelu.max-connection-queue-ms"
    ).toInt
  )
  val supaConfig = ServiceConfig(
    casUrl,
    supaServiceUrl,
    serviceUser,
    servicePassword,
    properties,
    maxSimultaneousConnections = findMandatoryPropertyValue(
      "suoritusrekisteri.suorituspalvelu.max-connections"
    ).toInt,
    maxConnectionQueueMs = findMandatoryPropertyValue(
  ...
  val tarjontaConfig = ServiceConfig(
    ...
    properties = properties,
    maxSimultaneousConnections =
      findMandatoryPropertyValue("suoritusrekisteri.tarjonta-service.max-connections").toInt,
    maxConnectionQueueMs =
  ...
  val koodistoConfig = ServiceConfig(
    ...
    properties = properties,
    maxSimultaneousConnections =
      findMandatoryPropertyValue("suoritusrekisteri.koodisto-service.max-connections").toInt,
    maxConnectionQueueMs =
      findMandatoryPropertyValue("suoritusrekisteri.koodisto-service.max-connection-queue-ms").toInt
  )
  val organisaatioConfig = ServiceConfig(
    serviceUrl = organisaatioServiceUrl,
    properties = properties,
    maxSimultaneousConnections =
      findMandatoryPropertyValue("suoritusrekisteri.organisaatio-service.max-connections").toInt,
    maxConnectionQueueMs = findMandatoryPropertyValue(
      "suoritusrekisteri.organisaatio-service.max-connection-queue-ms"
    ).toInt
  )

  val koskiConfig = ServiceConfig(
    serviceUrl = koskiServiceUrl,
    user = serviceUser,
    password = servicePassword,
    properties = properties,
    maxSimultaneousConnections =
      findMandatoryPropertyValue("suoritusrekisteri.koski.max-connections").toInt,
    maxConnectionQueueMs =
      findMandatoryPropertyValue("suoritusrekisteri.koski.max-connection-queue-ms").toInt
  )
  val valintaTulosConfig = new ServiceConfig(
    serviceUrl = valintaTulosServiceUrl,
    properties = properties,
    maxSimultaneousConnections =
  ...
  val valintarekisteriConfig = ServiceConfig(
    serviceUrl = valintaTulosServiceUrl,
    properties = properties,
    maxSimultaneousConnections =
      findMandatoryPropertyValue("suoritusrekisteri.valinta-tulos-service.max-connections").toInt,
  ...
  val oppijaNumeroRekisteriConfig = ServiceConfig(
    casUrl = casUrl,
    ...
    properties = properties,
    maxSimultaneousConnections = findMandatoryPropertyValue(
      "suoritusrekisteri.oppijanumerorekisteri-service.max-connections"
    ).toInt,
    maxConnectionQueueMs = findMandatoryPropertyValue(
      "suoritusrekisteri.oppijanumerorekisteri-service.max-connection-queue-ms"
    ).toInt
  )
  val valintaperusteetServiceConfig = ServiceConfig(
    casUrl = casUrl,
    ...
    properties = properties,
    maxSimultaneousConnections = findMandatoryPropertyValue(
      "suoritusrekisteri.valintaperusteet-service.max-connections"
    ).toInt,
    maxConnectionQueueMs = findMandatoryPropertyValue(
  ...
  val koodistoCacheHours =
    properties.getOrElse("suoritusrekisteri.cache.hours.koodisto", "12").toInt
  val organisaatioCacheHours =
    properties.getOrElse("suoritusrekisteri.cache.hours.organisaatio", "12").toInt
  val tarjontaCacheHours =
    properties.getOrElse("suoritusrekisteri.cache.hours.tarjonta", "12").toInt
  val valintatulosCacheHours =
    properties.getOrElse("suoritusrekisteri.cache.hours.valintatulos", "4").toInt
  val hakuRefreshTimeHours =
  ...
  val valpasValintatulosRefreshTimeHours =
    properties.getOrElse("suoritusrekisteri.refresh.time.hours.valpas.valintatulos", "12").toInt
  val valintatulosRefreshTimeHours =
    properties.getOrElse("suoritusrekisteri.refresh.time.hours.valintatulos", "2").toInt

  val asyncOperationThreadPoolSize: Int =
    properties.getOrElse("suoritusrekisteri.async.pools.size", "9").toInt
