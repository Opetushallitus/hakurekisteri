...
//  var retryRefresh: Option[Cancellable] = None
  ...
  private val cache = cacheFactory.getInstance[String, Organisaatio](
    timeToLive.toMillis,
    this.getClass,
    classOf[Organisaatio],
    "organisaatio"
  )
  private val childOidCache = cacheFactory.getInstance[String, ChildOids](
    timeToLive.toMillis,
    this.getClass,
    classOf[ChildOids],
    "child-oids"
  ...
  private def saveOrganisaatiot(s: Seq[Organisaatio]): Future[_] = {
    ...
    val recursiveSaveFuture = Future
      .sequence(s.map { org: Organisaatio =>
  ...
  private def findAndCacheChildOids(parentOid: String): Future[Option[ChildOids]] = {
    val tulos = organisaatioClient
      .readObject[ChildOids]("organisaatio-service.organisaatio.childoids", parentOid)(
        200,
        maxRetries
      )
      .map(Option(_))
    ...
    tulos.foreach { oids =>
      self ! CacheChildOids(parentOid, oids.get)
    }
      ...
      val organisationWithoutChildren: Future[Option[Organisaatio]] = organisaatioClient
        ...
        .recoverWith {
          ...
          case e: Exception if isCausedBy403(e) =>
            log.warning(
              s"Organisaatio forbidden with tunniste $tunniste. Ignoring the organization."
            )
            Future.successful(None)
          case o: Exception =>
            log.error(
              o,
              s"Unforeseen error occurred while fetching organisaatio with tunniste $tunniste"
            )
            throw o
        }

      val organisationWithChildren: Future[Option[Organisaatio]] =
        organisationWithoutChildren.flatMap {
  ...
  private def fetchAll(actor: ActorRef = ActorRef.noSender): Unit = {
    val all = organisaatioClient
      .readObject[OrganisaatioResponse]("organisaatio-service.hierarkia.hae")(200)
      .recoverWith { case t: Throwable =>
        Future.failed(OrganisaatioFetchFailedException(t))
      }
...
case class OppilaitosNotFoundException(koodi: String)
    extends Exception(s"Oppilaitosta ei löytynyt oppilaitoskoodilla $koodi.")
