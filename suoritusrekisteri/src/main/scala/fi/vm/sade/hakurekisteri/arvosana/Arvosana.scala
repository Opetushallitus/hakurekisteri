...
case class Arvosana(
  ...
  override def identify(identity: UUID): Arvosana with Identified[UUID] =
    new IdentifiedArvosana(this, identity)
...
case class KoskiUpdateable(
  arvio: Arvio,
  myonnetty: Option[LocalDate],
  lahdeArvot: Map[String, String]
)

case class KoskiCore(
  suoritus: UUID,
  aine: String,
  lisatieto: Option[String],
  valinnainen: Boolean,
...
class IdentifiedArvosana(a: Arvosana, val id: UUID)
    extends Arvosana(
      a.suoritus,
      a.arvio,
      a.aine,
...
object Arvio {
  ...
  def apply(arvosana: String, asteikko: String, pisteet: Option[Int] = None): Arvio =
    asteikko match {
