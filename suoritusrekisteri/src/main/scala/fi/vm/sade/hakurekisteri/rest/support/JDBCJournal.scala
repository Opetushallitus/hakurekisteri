...
object JDBCUtil {
  def createSchemaForTable[T <: Table[_]](table: lifted.TableQuery[T], db: Database)(implicit
    ec: ExecutionContext
  ) {
          ...
          .flatMap((t: Vector[MTable]) => {
            if (t.isEmpty) {
              schemaActionExtensionMethods(
                tableQueryToTableQueryExtensionMethods(table).schema
              ).create
            } else {
              DBIO.successful(())
            }
...
class JDBCJournal[R <: Resource[I, R], I, T <: JournalTable[R, I, _]](
  ...
  override def addModification(o: Delta[R, I]): Unit =
    runAsSerialized(10, 5.milliseconds, s"Storing $o", addModificationAction(o)).left
      .foreach(throw _)
  ...
  override def journal(latestQuery: Option[Long]): Seq[Delta[R, I]] = latestQuery match {
    ...
    case Some(lat) =>
      Await.result(db.run(latestResources.filter(_.inserted >= lat).result), queryTimeout)
  }
  ...
  def runAsSerialized[A](
    ...
    try {
      ...
      case e: PSQLException if e.getSQLState == SERIALIZATION_VIOLATION =>
        if (retries > 0) {
          log.warning(
            s"$description failed because of an concurrent action, retrying after $wait ms"
          )
