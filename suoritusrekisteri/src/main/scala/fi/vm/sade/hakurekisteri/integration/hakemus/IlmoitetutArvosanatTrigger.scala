...
object IlmoitetutArvosanatTrigger {
  ...
  implicit def osaaminen2RicherOsaaminen(osaaminen: Map[String, String]): RicherOsaaminen =
    RicherOsaaminen(osaaminen)
  implicit def koulutustausta2RicherKoulutustausta(
    koulutustausta: Map[String, String]
  ): RicherKoulutustausta = RicherKoulutustausta(koulutustausta)

  def arvosanaForSuoritus(arvosana: Arvosana, s: Suoritus with Identified[UUID]): Arvosana = {
    arvosana.copy(suoritus = s.id)
  }

  def muodostaSuorituksetJaArvosanat(
    ...
    def suoritusExists(suor: VirallinenSuoritus, suoritukset: Seq[Suoritus]): Boolean =
      suoritukset.exists {
        ...
        createSuorituksetJaArvosanatFromHakemus(hakemus).foreach {
          case (suor: VirallinenSuoritus, arvosanat) =>
            if (!suoritusExists(suor, suoritukset)) {
              for (suoritus: Suoritus with Identified[UUID] <- saveSuoritus(suor))
                arvosanat.foreach(arvosana =>
                  arvosanaRekisteri ! InsertResource[UUID, Arvosana](
                    arvosanaForSuoritus(arvosana, suoritus),
  ...
  def apply(suoritusRekisteri: ActorRef, arvosanaRekisteri: ActorRef)(implicit
    ec: ExecutionContext
  ): Trigger = {
    Trigger { (hakemus, personOidsWithAliases: PersonOidsWithAliases) =>
      {
        muodostaSuorituksetJaArvosanat(
          hakemus,
          suoritusRekisteri,
          arvosanaRekisteri,
  ...
  def aineArvotToArvosanat(personOid: String, aine: String, arvot: Map[String, String]) = {
    Seq(
      arvot
        .get("VAL1")
        .map(a =>
          Seq(createArvosana(personOid, arvot("VAL1"), aine, arvot.get("OPPIAINE"), true, Some(1)))
        )
        .getOrElse(Seq.empty),
      arvot
        .get("VAL2")
        .map(a =>
          Seq(createArvosana(personOid, arvot("VAL2"), aine, arvot.get("OPPIAINE"), true, Some(2)))
        )
        .getOrElse(Seq.empty),
      arvot
        .get("VAL3")
        .map(a =>
          Seq(createArvosana(personOid, arvot("VAL3"), aine, arvot.get("OPPIAINE"), true, Some(3)))
        )
        .getOrElse(Seq.empty),
      arvot
        .get("")
        .map(a => Seq(createArvosana(personOid, arvot(""), aine, arvot.get("OPPIAINE"), false)))
        .getOrElse(Seq.empty)
    ).flatten
  }

  def createSuorituksetJaArvosanatFromHakemus(
    hakemus: HakijaHakemus
  ): Seq[(Suoritus, Seq[Arvosana])] = hakemus match {
    case hakemus: FullHakemus =>
      (for (
        ...
        try {
          ...
          val pkLisapiste =
            createPkLisapisteSuoritukset(hakemus, personOid, answers, koulutustausta)
          ...
          case anyException: Throwable => {
            ...
            throw new Exception(
              "Collecting peruskouluarvosanat from hakemus " + hakemus.oid + " with personID " + personOid,
              anyException
            )
          }
        }
  ...
  def createPkSuoritusArvosanat(
    hakemus: FullHakemus,
    personOid: String,
    answers: HakemusAnswers,
    koulutustausta: Koulutustausta
  ): Seq[(Suoritus, Seq[Arvosana])] = {
    (for (
      ...
      val arvosanat: Seq[Arvosana] = answers.osaaminen match {
        case Some(osaaminen) =>
          osaaminen.getPeruskoulu
            .map({ case (aine, arvot) => aineArvotToArvosanat(personOid, aine, arvot) })
            .flatten
            .toSeq
        case None => Seq.empty
      }
  ...
  def createLukioSuoritusArvosanat(
    hakemus: FullHakemus,
    personOid: String,
    answers: HakemusAnswers,
    koulutustausta: Koulutustausta
  ...
  def createPkLisapisteSuoritukset(
    hakemus: FullHakemus,
    personOid: String,
    answers: HakemusAnswers,
    koulutustausta: Koulutustausta
    ...
    (for (
      ...
      Seq(
        // AMMATTISTARTTI
        koulutustausta.LISAKOULUTUS_AMMATTISTARTTI
          .map(lk => {
            if ("true".equals(lk)) {
              Seq(
                ItseilmoitettuTutkinto(
          ...
          .getOrElse(Seq.empty),
        // LISÄOPETUSTALOUS
        koulutustausta.LISAKOULUTUS_TALOUS
          .map(lk => {
            if ("true".equals(lk)) {
              Seq(
                ItseilmoitettuTutkinto(
          ...
          .getOrElse(Seq.empty),
        // LISAOPETUSTUTKINTO
        koulutustausta.LISAKOULUTUS_KYMPPI
          .map(lk => {
            if ("true".equals(lk)) {
              Seq(
                ItseilmoitettuTutkinto(
                  ...
                  hakemusOid = hakemus.oid,
                  hakijaOid = personOid,
                  koulutustausta.KYMPPI_PAATTOTODISTUSVUOSI
...
case class RicherOsaaminen(osaaminen: Map[String, String]) {
  val groupByKomoAndGroupByAine = osaaminen
    .filterKeys(_.contains("_"))
    .filterKeys(!_.last.equals('_'))
    ...
    .map({ case (key, value) =>
      (
        key,
        value
          .groupBy({ case (k, v) => k.split("_").head })
          .mapValues(vals =>
            vals
              .map({ case (kxx, vxx) =>
                (stringAfterFirstUnderscore(kxx), vxx)
              })
              .filter(v => {
                if (v._1.equals("")) {
          ...
          .filter(v => {
            v._2.contains("") || v._2.contains("VAL1") || v._2.contains("VAL2") || v._2
              .contains("VAL3")
          })
      )
    })
  ...
  private def stringAfterFirstUnderscore(source: String): String =
    if (!source.contains("_")) "" else source.substring(source.indexOf("_") + 1)
  private def isAllDigits(x: Option[String]) =
    if (x.isEmpty) true else x.get forall Character.isDigit
