...
trait JDBCRepository[R <: Resource[I, R], I, T <: JournalTable[R, I, _]] extends Repository[R, I] {
  ...
  def latest(id: I): lifted.Query[T, Delta[R, I], Seq] =
    all.filter((item) => item.resourceId === id)
  ...
  private def joinHenkilotWithTempTable(
    ...
    joinOn: String,
    baseQuery: lifted.Query[T, Delta[R, I], Seq]
  ): DBIOAction[Seq[Delta[R, I]], Streaming[Delta[R, I]], All with Transactional] = {
    ...
    val createHenkiloviiteTempTable: SimpleDBIO[Array[Int]] with Dumpable =
      new SimpleJdbcAction({ session =>
        ...
        override def getDumpInfo: DumpInfo = DumpInfo(
          ...
          mainInfo = "[" + createTempTableStatements + "]"
        )
      }
    ...
    val selectAllMatching = for {
      (record, _) <- baseQuery join henkiloviiteTempTable on (_.column[String](
        joinOn
      ) === _.linkedOid)
    } yield record

    createHenkiloviiteTempTable
      .andThen(populateTempTable)
      .andThen(selectAllMatching.distinct.result)
      .transactionally
  }

  def findWithHenkilot(
    henkilot: PersonOidsWithAliases,
    joinOn: String,
    baseQuery: lifted.Query[T, Delta[R, I], Seq]
  ): DBIOAction[Seq[Delta[R, I]], Streaming[Delta[R, I]], All with Transactional] = {
    henkilot.uniquePersonOid match {
      case Some(uniquePersonOid) =>
        baseQuery.filter(x => x.column[String](joinOn) === uniquePersonOid).result
      case None => joinHenkilotWithTempTable(henkilot, joinOn, baseQuery)
    }
  }

  override def get(id: I): Option[R with Identified[I]] =
    Await.result(journal.db.run(latest(id).result.headOption), 10.seconds).collect {
      case Updated(res) => res
    }

  override def getAll(ids: Seq[I]): Seq[R with Identified[I]] =
    Await.result(journal.db.run(all.filter(_.resourceId.inSet(ids)).result), 120.seconds).collect {
      case Updated(res) => res
    }

  override def listAll(): Seq[R with Identified[I]] =
    Await.result(journal.db.run(all.result), 1.minute).collect { case Updated(res) =>
      res
    }

  def deduplicationQuery(i: R)(t: T): lifted.Rep[Boolean]
  def deduplicationQuery(i: R, p: Option[PersonOidsWithAliases])(t: T): lifted.Rep[Boolean] =
    deduplicationQuery(i)(t)

  private def deduplicate(
    i: R,
    p: Option[PersonOidsWithAliases]
  ): DBIO[Option[R with Identified[I]]] = all
    .filter(deduplicationQuery(i, p))
    .result
    .map(_.collect { case Updated(res) =>
      res
    }.headOption)
  ...
  protected def doSave(
    ...
    p: Option[PersonOidsWithAliases] = None
  ): R with Identified[I] = {
    journal.runAsSerialized(
      10,
      5.milliseconds,
      s"Saving $i",
  ...
  ): Future[R with Identified[I]] = {
    Future.successful(
      doSave(t, (i, old) => journal.addUpdate(i.identify(old.id)), Some(personOidsWithAliases))
    )
  }
  ...
  override def insert(t: R, personOidsWithAliases: PersonOidsWithAliases): R with Identified[I] =
    journal.runAsSerialized(
      ...
      5.milliseconds,
      s"Inserting $t",
      deduplicate(t, Some(personOidsWithAliases))
        .flatMap(_.fold(journal.addUpdate(t.identify))(DBIO.successful))
    ) match {
      case Right(r) => r
      case Left(e)  => throw e
    }
...
trait JDBCService[R <: Resource[I, R], I, T <: JournalTable[R, I, _]]
    extends ResourceService[R, I] { this: JDBCRepository[R, I, T] with ActorLogging =>
  ...
  val dbQuery: PartialFunction[
    Query[R],
    Either[Throwable, DBIOAction[Seq[Delta[R, I]], Streaming[Delta[R, I]], All]]
  ]

  private def logSlowQuery(runtime: Long, query: DBIOAction[_, _, _]): Unit = {
    val sqlsAndCounts: Seq[(String, Int)] = sqlOf(query)
    val sqlStringSeq =
      sqlsAndCounts.map(x => x._1 + (if (x._2 > 1) s" (repeated ${x._2} times)" else ""))
    val sqlString = sqlStringSeq.mkString(" ")

    val (loggableQueryStr, prettyPrint) =
      if (sqlString.length > journal.dbLoggingConfig.maxLogLineLength) {
        (
          sqlString.take(
            journal.dbLoggingConfig.maxLogLineLength
          ) + "...(truncated from " + sqlString.length + " chars)",
