...
object HakurekisteriDriver extends HakurekisteriDriver1 {
  override val columnTypes: HakurekisteriDriver.JdbcTypes = new super.JdbcTypes {
    override val uuidJdbcType: UUIDJdbcType = new UUIDJdbcType {
      ...
      override def setValue(v: UUID, p: PreparedStatement, idx: Int): Unit =
        p.setObject(idx, v.toString, sqlType)
      override def updateValue(v: UUID, r: ResultSet, idx: Int): Unit =
        r.updateObject(idx, v.toString)
  ...
  val startOfYear = SimpleExpression.unary[String, DateTime] { case (year, qb) =>
    qb.sqlBuilder += "extract(epoch from to_timestamp("
    qb.expr(year)
    qb.sqlBuilder += " || '-01-01', 'YYYY-MM-DD')) * 1000"
  }

  val startOfAutumnDate = SimpleExpression.unary[String, LocalDate] { case (year, qb) =>
    qb.expr(year)
    qb.sqlBuilder += " || '-08-01'"
  }

  val startOfAutumn = SimpleExpression.unary[String, DateTime] { case (year, qb) =>
    qb.sqlBuilder += "extract(epoch from to_timestamp("
    qb.expr(year)
  ...
  def yearOf(c: Rep[DateTime]): Rep[String] =
    SimpleExpression.unary[DateTime, String](yearOfExpr).apply(c)

  def yearOf(c: Rep[Option[DateTime]])(implicit d: DummyImplicit): Rep[String] =
    SimpleExpression.unary[Option[DateTime], String](yearOfExpr).apply(c)

  def yearOf(
    c: Rep[Option[LocalDate]]
  )(implicit d1: DummyImplicit, d2: DummyImplicit): Rep[String] = SimpleExpression
    .unary[Option[LocalDate], String]({ case (date, qb) =>
      qb.sqlBuilder += "extract(year from to_timestamp("
      qb.expr(date)
