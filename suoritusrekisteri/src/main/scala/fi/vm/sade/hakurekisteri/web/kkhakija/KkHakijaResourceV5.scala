...
class KkHakijaResourceV5(kkHakijaService: KkHakijaService, ophConfig: Config)(implicit
  system: ActorSystem,
  sw: Swagger,
  val security: Security,
  val ct: ClassTag[Seq[Hakija]]
) extends HakuJaValintarekisteriStack
    with KkHakijaSwaggerApiV5
  ...
  get("/", operation(query)) {
    ...
    val kkhakijatFuture = kkHakijaService.getKkHakijat(q, 5).flatMap {
      case result
