...
class HakijaResourceV5(hakijaActor: ActorRef)(implicit
  ...
  override val streamingRender: (OutputStream, JSONHakijatV5) => Unit = (out, hakijat) => {
    ExcelUtilV5.write(out, hakijat)
  }

  get("/", operation(queryV5)) {
    val q = HakijaQuery(params.toMap, currentUser, 5)
