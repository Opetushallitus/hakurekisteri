...
trait OppijaFetcher {
  ...
  def fetchOppijat(ensikertalaisuudet: Boolean, q: HakemusQuery)(implicit
    user: User
  ): Future[Seq[Oppija]] = {
    for (
      personOids <- q.hakukohde match {
        case Some(hakukohdeOid) =>
          hakemusService.personOidsForHakukohde(hakukohdeOid, q.organisaatio)
  ...
  def fetchOppijat(persons: Set[String], ensikertalaisuudet: Boolean, q: HakemusQuery)(implicit
    user: User
  ): Future[Seq[Oppija]] = {
    oppijaNumeroRekisteri
      .enrichWithAliases(persons)
      .flatMap(personOidsWithAliases => {
  ...
  def fetchOppija(person: String, ensikertalaisuudet: Boolean, hakuOid: Option[String])(implicit
    user: User
  ): Future[Oppija] = {
    fetchOppijat(Set(person), ensikertalaisuudet, HakemusQuery(haku = hakuOid))(user).map(_.head)
  }

  def getRekisteriData(
    personOidsWithAliases: PersonOidsWithAliases
  )(implicit user: User): Future[Seq[Oppija]] = {
    ...
    val todistuksetF =
      timed("Suoritukset for rekisteritiedot", fetchSuoritukset(personOidsWithAliases))
        .flatMap(suoritukset =>
          timed("Todistukset for rekisteritiedot", fetchTodistukset(suoritukset))
        )
        .map(_.groupBy(_.suoritus.henkiloOid))
    val opiskeluoikeudetF =
      timed("Opiskeluoikeudet for rekisteritiedot", fetchOpiskeluoikeudet(personOidsWithAliases))
        .map(_.groupBy(_.henkiloOid))
    val opiskelijatF =
      timed("Opiskelijat for rekisteritiedot", fetchOpiskelu(personOidsWithAliases))
        .map(_.groupBy(_.henkiloOid))
    ...
    } yield personOidsWithAliases.henkiloOids
      .map(henkiloOid =>
        Oppija(
          ...
          suoritukset = suorituksetWithAliases(
            personOidsWithAliases.intersect(Set(henkiloOid)),
            todistukset,
            henkiloOid
          ),
  ...
  private def suorituksetWithAliases(
    personOidsWithAliases: PersonOidsWithAliases,
    todistuksetByPersonOid: Map[String, Seq[Todistus]],
    oid: String
  ): Seq[Todistus] = {
  ...
  private def fetchTodistukset(
    suoritukset: Seq[Suoritus with Identified[UUID]]
  )(implicit user: User): Future[Seq[Todistus]] =
    for (
      arvosanat <- (rekisterit.arvosanaRekisteri ? AuthorizedQuery(
        ArvosanatQuery(suoritukset.map(_.id).toSet),
        user
      ))
        .mapTo[Seq[Arvosana]]
  ...
  private def fetchEnsikertalaisuudet(
    q: HakemusQuery
    ...
    for (
      ...
      yield for (oppija <- rekisteriData)
        yield oppija.copy(ensikertalainen =
          ensikertalaisuudet.get(oppija.oppijanumero).map(_.ensikertalainen)
        )
  }

  private def fetchOpiskeluoikeudet(
    personOidsWithAliases: PersonOidsWithAliases
  )(implicit user: User): Future[Seq[Opiskeluoikeus]] =
    splittedQuery[Opiskeluoikeus, Opiskeluoikeus](
      personOidsWithAliases,
      rekisterit.opiskeluoikeusRekisteri,
      (henkilot) => OpiskeluoikeusHenkilotQuery(henkilot)
    )

  private def fetchOpiskelu(
    personOidsWithAliases: PersonOidsWithAliases
  )(implicit user: User): Future[Seq[Opiskelija]] =
    splittedQuery[Opiskelija, Opiskelija](
      personOidsWithAliases,
      rekisterit.opiskelijaRekisteri,
      (henkilot) => OpiskelijaHenkilotQuery(henkilot)
    )

  private def fetchSuoritukset(
    personOidsWithAliases: PersonOidsWithAliases
  )(implicit user: User): Future[Seq[Suoritus with Identified[UUID]]] =
    splittedQuery[Suoritus with Identified[UUID], Suoritus](
      personOidsWithAliases,
      rekisterit.suoritusRekisteri,
      (henkilot) => SuoritusHenkilotQuery(henkilot)
    )

  private def splittedQuery[A, B](
    personOidsWithAliases: PersonOidsWithAliases,
    actor: ActorRef,
    q: (PersonOidsWithAliases) => Query[B]
  )(implicit user: User): Future[Seq[A]] =
    Future
      .sequence(
        personOidsWithAliases.henkiloOids
          .grouped(singleSplitQuerySize)
          .map(henkiloSubset =>
            (actor ? AuthorizedQuery(q(personOidsWithAliases.intersect(henkiloSubset)), user))
              .mapTo[Seq[A]]
          )
      )
      .map(_.flatten.toSeq)
