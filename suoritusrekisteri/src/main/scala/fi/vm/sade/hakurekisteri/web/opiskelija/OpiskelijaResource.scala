...
class OpiskelijaResource(opiskelijaActor: ActorRef)(implicit
  sw: Swagger,
  s: Security,
  system: ActorSystem
  ...
  override def parseResourceFromBody(user: String): Either[ValidationError, Opiskelija] = {
    try {
      ...
      val errors = checkMandatory(
        Seq("oppilaitosOid", "luokkataso", "luokka", "henkiloOid", "alkuPaiva"),
        bodyValues
      )
      ...
      Right(
        Opiskelija(
          ...
          loppuPaiva =
            if (bodyValues.contains("loppuPaiva") && !JNull.equals(bodyValues("loppuPaiva")))
              Some(DateTime.parse(bodyValues("loppuPaiva").extract[String]))
            else None,
          source = user
        )
      )
