    ...
    muokkaaHenkilo: (henkiloOid, $scope) ->
      ...
      loadMenuTexts = (komo) ->
        $scope.koulutukset = [
          ...
          {
            value: komo.tuvakoulutuksensuoritus
            text: getOphMsg("suoritusrekisteri.komo." + komo.tuvakoulutuksensuoritus, "Tutkintokoulutukseen valmentava koulutus")
          }
        ]
      ...
      fetchKomos = ->
        ...
        $http.get(window.url("suoritusrekisteri.komo"), { cache: true }).success((data) ->
          ...
          komosLoaded.resolve
            ...
            tuvakoulutuksensuoritus: data.tuvaKomoOid
        ...
        if Array.isArray($scope.myRoles)
          ...
          $scope.showKoskiLink = () ->
            $scope.myRoles.indexOf("APP_KAYTTOOIKEUS_REKISTERINPITAJA") > -1 and
              ...
              $scope.myRoles.indexOf("APP_KOSKI_OPHPAAKAYTTAJA") > -1 or
